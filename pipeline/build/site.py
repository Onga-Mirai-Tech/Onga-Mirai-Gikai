...
OPERATOR = "Onga-Mirai-Tech（個人運営）"
CONTACT = "info@onga-mirai-tech.com"
# 施行日。変えたら「改訂」として日付を足す。
POLICY_ENACTED = "2026年9月24日"


def contact_block() -> str:
    return (
        '<div class="referral">'
        f"<b>{esc(OPERATOR)}</b><br>"
        f'メール：<a href="mailto:{CONTACT}">{CONTACT}</a>'
        "</div>"
    )


def legal_page(site: Site, *, title: str, lead: str, sections: list[tuple[str, str]],
               description: str) -> str:
    """規約類のページ。見出しと本文の組を並べるだけの、長文を読むための組み方。"""
    depth = 1
    body = (
        crumb(depth, (title, ""))
        + f"<h1>{esc(title)}</h1>"
        + f'<p class="lead">{lead}</p>'
        + "".join(f'<h2>{esc(h)}</h2><div class="prose">{html}</div>' for h, html in sections)
        + "<h2>お問い合わせ先</h2>" + contact_block()
        + f'<p class="meta enacted">{POLICY_ENACTED} 制定</p>'
    )
    return page(site, title=title, body=body, depth=depth, description=description)
    ...
    **実際に集めているものだけを書く。** 2026-09-24 に確認した事実:
    ...
    sections = [
        ("1. 本サイトが集める情報",
         ...
         "IPアドレス、閲覧した日時とページ、ブラウザの種類、参照元のページなどです。"
         "サーバーの機能による閲覧数の集計にも使われます。</li>"
         "<li><b>お問い合わせの内容</b>　メールでお問い合わせいただいた場合の、メールアドレス、お名前（書かれていれば）、お問い合わせの内容。</li>"
         "</ol>"),
        ("2. 使いみち",
         "<ul>"
         "<li>アクセスログは、サーバーの障害への対応、不正なアクセスの調査、閲覧数の把握に使います。"
         "閲覧数はページごとの件数などの形で集計し、個人を特定する目的では使いません。</li>"
         "<li>お問い合わせの内容は、回答と、ご指摘いただいた誤りの訂正に使います。</li>"
         "</ul>"
         "<p>これ以外の目的には使いません。</p>"),
        ("3. 第三者への提供",
         "<p>法令に基づく場合を除き、第三者に提供しません。</p>"
         "<p>なお、アクセスログはサーバー会社の設備に記録されます。"
         "サーバー会社による取り扱いは、同社のプライバシーポリシーに従います。</p>"),
        ("4. AI（人工知能）の利用について",
         "<p>本サイトのAI要約は、<b>遠賀町議会が公開している会議録だけをもとに</b>作成しています。"
         "利用者の情報（アクセスログやお問い合わせの内容）をAIに送ることはありません。"
         "本サイトにはAIと対話する機能もありません。</p>"),
        ("5. 会議録に含まれる氏名について",
         "<p>本サイトは、遠賀町議会の会議録に記載された発言を掲載しています。"
         "議員、町長をはじめとする町の職員の氏名は、会議録の記載のとおりに掲載します。</p>"
         "<p>それ以外の方で、会議録に記載されたご自身の氏名などの掲載について訂正や削除を希望される場合は、"
         "下記のお問い合わせ先までご連絡ください。内容を確認のうえ、個別に対応します。</p>"),
        ("6. 管理と保管",
         "<p>お問い合わせのメールは、運営者だけが閲覧できる環境で管理し、"
         "対応に必要な期間を過ぎたものから削除します。</p>"),
        ("7. 改訂",
         "<p>内容を変更する場合は、このページに改訂日とともに掲載します。</p>"),
    ]
    return legal_page(
        site, title="プライバシーポリシー",
        lead=f"みらい議会＠遠賀町（以下「本サイト」）を運営する{esc(OPERATOR)}（以下「運営者」）は、"
             "本サイトの利用者の情報を次のとおり取り扱います。",
        sections=sections,
        description="本サイトが集める情報（アクセスログとお問い合わせのみ）と、その扱い。",
    )


def render_terms(site: Site) -> str:
    """利用規約。

    再利用は「引用の範囲」にとどめる（2026-09-24決定）。AI要約は会議録の翻案で、
    ...
    sections = [
        ("第1条（本サイトについて）",
         "<ol>"
         "<li>本サイトは、遠賀町議会の会議録をもとに議事の流れを見やすく整理した、個人が運営する非公式のサイトです。</li>"
         "<li>遠賀町・遠賀町議会が公式に提供しているものではありません。</li>"
         f"<li>{esc(FOOTER_PARTY_HEAD)}。{esc(FOOTER_PARTY_BODY)}</li>"
         "</ol>"),
        ("第2条（AI要約について）",
         "<ol>"
         "<li>本サイトには、AIが会議録から作成した要約が含まれます。AIが作成した部分はその旨を表示し、"
         "根拠となった発言の原文へのリンクを付けています。</li>"
         "<li>AI要約には誤りが含まれる可能性があります。正確な内容は、必ず原典（遠賀町議会 会議録検索システム）でご確認ください。"
         "<b>本サイトの記載と原典が異なる場合は、原典が優先します。</b></li>"
         "</ol>"),
        ("第3条（著作権）",
         "<ol>"
         "<li>本サイトに掲載している会議録の発言、一般質問通告書および審議案件・結果の内容の著作権は、"
         "遠賀町議会その他のそれぞれの権利者に帰属します。</li>"
         "<li>AI要約、本サイトの説明文およびページの構成は、運営者が作成したものです。"
        ...
        ("第4条（リンク）",
         ...
         "ただし、本サイトが遠賀町・遠賀町議会または特定の政党・団体と関係があるかのように誤解させる方法でのリンクはお控えください。</p>"),
        ("第5条（誤りの指摘）",
         "<p>本サイトの内容に誤りを見つけられた場合は、お問い合わせ先までご連絡ください。確認のうえ訂正します。</p>"),
        ("第6条（禁止事項）",
         "<p>本サイトの利用にあたり、次の行為を禁止します。</p>"
         "<ol>"
         "<li>本サイトのサーバーに過度な負荷をかける行為（短時間に大量のページを自動で取得することなど）</li>"
         "<li>本サイトの内容を改変したうえで、本サイトまたは原典の記載であるかのように示す行為</li>"
         "<li>法令に違反する行為</li>"
         "</ol>"),
        ("第7条（免責）",
         "<ol>"
         "<li>運営者は、本サイトの内容の正確性、完全性および有用性について保証しません。</li>"
         "<li>本サイトの利用によって生じた損害について、運営者は責任を負いません。"
         "ただし、運営者の故意または重大な過失による場合はこの限りではありません。</li>"
         "<li>本サイトからリンクしている外部のサイトの内容について、運営者は責任を負いません。</li>"
         "</ol>"),
        ("第8条（サービスの変更・停止）",
         "<p>運営者は、予告なく本サイトの内容を変更し、または運営を停止することがあります。</p>"),
        ("第9条（規約の変更）",
         "<p>運営者は、必要に応じて本規約を変更することがあります。変更後の規約は、このページに掲載した時点から効力を生じます。</p>"),
        ("第10条（準拠法・管轄）",
         "<p>本規約は日本法に基づいて解釈されます。本サイトに関して紛争が生じた場合は、"
         "福岡地方裁判所を第一審の専属的合意管轄裁判所とします。</p>"),
    ]
    return legal_page(
        site, title="利用規約",
        lead="この利用規約（以下「本規約」）は、みらい議会＠遠賀町（以下「本サイト」）の利用について定めるものです。"
             "本サイトを利用された方は、本規約に同意したものとみなします。",
        sections=sections,
